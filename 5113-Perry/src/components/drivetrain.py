...
class Drivetrain(AdaptiveComponent):
    ...
    def __init__(self, robot: "AdaptiveRobot") -> None:
        ...
        self.rotation_slew = SlewRateLimiter(DrivetrainConst.SLEW_ROTATION)

        self.forward_axis_controller = AxisController()
        self.rotation_axis_controller = AxisController()
    ...
    def get_heading(self) -> Rotation2d:
        """Returns the robot CCW heading as a Rotation2d (specific hardware configuration)."""
    ...
    def request_stop(self) -> None:
        ...
        self.forward_axis_controller.request(
            AxisRequest(
                value=0.0, 
                priority=DrivePriority.SAFETY.value,
                source="safety"
            )
        )
        self.rotation_axis_controller.request(
            AxisRequest(
                value=0.0, 
                priority=DrivePriority.SAFETY.value,
                source="safety"
    ...
    def request_forward_percent(
        ...
        forward = self.forward_slew.calculate(forward)
    ...
    def request_rotation_percent(
        ...
        rotation = self.rotation_slew.calculate(rotation)
    ...
    def _tank_drive_velocity(
        ...
        # Convert left velocity in rpm to rps, then to mps
        measured_left_mps = rps_to_mps(
            motor_rps=rpm_to_rps(left_velocity),
            wheel_diameter=RobotConst.WHEEL_DIAMETER,
            motor_to_wheel_ratio=DrivetrainConst.GEAR_RATIO
        )
        measured_right_mps = rps_to_mps(
            motor_rps=rpm_to_rps(right_velocity),
            wheel_diameter=RobotConst.WHEEL_DIAMETER,
            motor_to_wheel_ratio=DrivetrainConst.GEAR_RATIO
        )
    ...
    def publish_telemetry(self) -> None:
        ...
        resolved_vx_request = self.forward_axis_controller.resolve()
        resolved_omega_request = self.rotation_axis_controller.resolve()
    ...
    def execute(self) -> None:
        ...
        vx = self.forward_axis_controller.resolve().value
        omega = self.rotation_axis_controller.resolve().value
        ...
        self.forward_axis_controller.clear()
        self.rotation_axis_controller.clear()
